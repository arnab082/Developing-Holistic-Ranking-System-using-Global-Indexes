---
title: "JCOMP - Human Development Score"
...
```{r}
rm(list = ls())
data<-read.csv("/home/soham/Downloads/happiness_data.csv")
data <- na.omit(data)
data$Gross.national.income..GNI..per.capita <- as.numeric(gsub(",","",data$Gross.national.income..GNI..per.capita))
rownames(data)<-data$Country
data$Country <- NULL
library(plyr)
data$Country <- as.character(mapvalues(rownames(data), 
                                       from = c("Hong Kong, China (SAR)","Korea (Republic of)","United Kingdom","United States", "Antigua and Barbuda","Czechia","Türkiye","Brunei Darussalam","Russian Federation","Trinidad and Tobago","Trinidad and Tobago","Saint Kitts and Nevis","Iran (Islamic Republic of)","Moldova (Republic of)","Saint Vincent and the Grenadines","Palestine, State of","Viet Nam" ,"Bolivia (Plurinational State of)","Venezuela (Bolivarian Republic of)","Cabo Verde","Micronesia (Federated States of)","Lao People's Democratic Republic","Eswatini (Kingdom of)","Syrian Arab Republic","Tanzania (United Republic of)","Côte d'Ivoire","Congo (Democratic Republic of the)","Congo"),
                                       to = c("Hong Kong","South Korea","UK","USA","Antigua","Czech Republic","Turkey","Brunei","Russia","Trinidad","Tobago","Saint Kitts","Iran","Moldova","Saint Vincent","West Bank and Gaza","Vietnam","Bolivia","Venezuela","Cape Verde","Micronesia","Laos","Swaziland","Syria","Tanzania","Cote d'Ivoire","Democratic Republic of the Congo","Republic of Congo")))
rownames(data)<-data$Country
unique(rownames(data))
library(dplyr)
data = select(data,-c('SL.No'))
head(data)
View(data)
summary(data)
```
...
library(dplyr)
dta = select(dta,-c('Human.Development.Index..HDI.'))
...
HDI_Score<-data$Human.Development.Index..HDI.

dta<-data.frame(my_pca$x,HDI_Score=HDI_Score)
head(dta)
...
ggplot() +
  ...
  labs(title = "Applied Clustering Happiness Index",
       x = NULL, y = NULL) +
  coord_equal() +
